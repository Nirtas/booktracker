...
class UpdateBookCoverUseCaseTest {
    ...
    @Test
    fun `when everything is fine, the cover is successfully updated`() = runBlocking {
        coEvery { getBookByIdUseCase(bookId, language) } returns existingBookWithCover
    ...
    fun `when a book is not found, a BookNotFoundException should be thrown`() = runBlocking {
        coEvery { getBookByIdUseCase(bookId, language) } throws BookNotFoundException(bookId.toString())
    ...
    @Test
    fun `when it is not possible to save a new cover to storage, a StorageException should be thrown`() = runBlocking {
        coEvery { getBookByIdUseCase(bookId, language) } returns existingBookWithCover
        coEvery { coverStorage.delete(oldCoverPath) } just Runs
    ...
    @Test
    fun `when repository fails to update book, it should propagate the exception`() = runBlocking {
        coEvery { getBookByIdUseCase(bookId, language) } returns existingBookWithCover
        coEvery { coverStorage.delete(oldCoverPath) } just Runs
        coEvery { coverStorage.save(coverPart) } returns newCoverPath
        coEvery {
    ...
    @Test
    fun `when the book does not have a cover, it is not deleted`() = runBlocking {
        coEvery { getBookByIdUseCase(bookId, language) } returns existingBookWithoutCover
        coEvery { coverStorage.save(coverPart) } returns newCoverPath
        coEvery { bookRepository.updateBookCover(bookId, newCoverPath, language) } returns updatedBook
