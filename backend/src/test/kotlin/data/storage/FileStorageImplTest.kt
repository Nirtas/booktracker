...
class FileStorageImplTest {
    ...
    @Test
    fun `when saveFile is called with valid data, it should create file and write content to it`() = runBlocking {
    ...
    @Test
    fun `when saveFile is called with an empty ByteReadChannel, a ValidationException should be thrown`() =
        runBlocking {
            val path = "test/file.txt"
            val content = ""
            val channel = ByteReadChannel(content)
    ...
    @Test
    fun `when saveFile is called and disk write fails, a StorageException should be thrown`() = runBlocking {
        val parentPath = "test"
    ...
    @Test
    fun `when deleteFile is called with an existing file path, it should remove the file from disk`() = runBlocking {
        val path = "file.txt"
        val file = File(tempDir, path)
    ...
    @Test
    fun `when deleteFile is called with an non-existing file path, it should complete without errors`() = runBlocking {
        val path = "file.txt"
    ...
    @Test
    fun `when deleteFile is called on a non-empty directory, a StorageException should be thrown`() = runBlocking {
        val dirPath = "test"
